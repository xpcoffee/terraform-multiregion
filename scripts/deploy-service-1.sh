...
echo "Deploying service-1 to region: ${REGION}, stage: ${STAGE}"
...
echo "Restarting service-1 deployment..."
kubectl rollout restart deployment/service-1 -n "${NAMESPACE}"

echo "Waiting for service-1 rollout to complete..."
kubectl rollout status deployment/service-1 -n "${NAMESPACE}" --timeout=300s

echo "Getting service-1 pods status..."
kubectl get pods -n "${NAMESPACE}" -l app=service-1

echo "Service-1 deployment completed successfully!"
echo "You can check the service with: kubectl get service service-1 -n ${NAMESPACE}"