...
"${SCRIPT_DIR}/destroy-component.sh" service-2 "${REGION}" "${STAGE}" || true
echo ""

# Destroy service-1 (Service team 1)
echo "Step 2/4: Destroying service-1 stamp..."
"${SCRIPT_DIR}/destroy-component.sh" service-1 "${REGION}" "${STAGE}" || true
echo ""

# Destroy postgres (Database team)
echo "Step 3/4: Destroying PostgreSQL stamp..."
"${SCRIPT_DIR}/destroy-component.sh" postgres "${REGION}" "${STAGE}" || true
echo ""

# Destroy cluster (Platform team)
echo "Step 4/4: Destroying cluster stamp..."
"${SCRIPT_DIR}/destroy-component.sh" cluster "${REGION}" "${STAGE}" || true
echo ""
